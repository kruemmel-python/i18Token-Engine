...
FANTASY_STATE_FILE = BASE_DIR / "fantasy_state.json"


def set_active_catalog_context(catalog_path: Path):
    global ACTIVE_CATALOG, RUNTIME_CATALOG, RUNTIME_CATALOG_BIN, SAVEGAME_FILE, LOGBOOK_FILE, FANTASY_STATE_FILE
    ...
    FANTASY_STATE_FILE = BASE_DIR / f"fantasy_state_{suffix}.json"
...
def parse_catalog_lines(raw_text: str, target: dict):
    for line in raw_text.splitlines():
        ...
        target[key.strip().lstrip("\ufeff")] = value.strip()
...
def ensure_selection_cache():
    ...
    if not BASE_CATALOG.exists():
        return
    parse_catalog_lines(BASE_CATALOG.read_text(encoding="utf-8-sig"), SELECTION_CACHE)
...
def translate_cached_token(token: str, cache: dict, args=None):
    ...
    if args:
        for idx, arg in enumerate(args):
            ...
            text = text.replace("{" + str(idx) + "}", str(arg))
...
SECTOR_RESOURCE_NODES = {
    "000W10": ["000N50", "000N51", "000N52", "000N55"],
    "000W11": ["000N52", "000N53", "000N54", "000N56", "000N57"],
}
DEFAULT_RESOURCE_POOL = ["000N50", "000N51", "000N52", "000N53", "000N54", "000N55", "000N56", "000N57"]
...
RECIPE_TOKENS = ["000C100", "000C101", "000C102", "000C103", "000C104"]
...
class Player:
    ...
    def save_game(self, engine):
        ...
        with SAVEGAME_FILE.open("w", encoding="utf-8") as f:
            f.write(data)
        if fantasy_enabled():
            state = load_fantasy_state()
            fantasy_log(state, f"{self.name} sicherte die Sigillen-Chronik.")
            save_fantasy_state(state)
...
# ---------------------------------------------------------------------------
# FANTASY EXPANSION LAYER
# ---------------------------------------------------------------------------

def fantasy_enabled():
    return ACTIVE_CATALOG.stem.startswith("fantasy")


def fantasy_default_state():
    return {
        "version": 2,
        "region_states": {
            "000W10": {"stability": 92, "threat": 1, "omen": ""},
            "000W11": {"stability": 78, "threat": 2, "omen": ""},
        },
        "world_rupture": 12,
        "current_omen": "",
        "omen_turns": 0,
        ...
        "bestiary": {},
        "active_title": "",
        "path_counts": {"heavy": 0, "heal": 0, "craft": 0, "event": 0, "faction": 0, "scan": 0},
        "class_path": "",
        "relics": [],
        "equipped_relic": "",
        "chronicle": [],
...
def load_fantasy_state():
    if not fantasy_enabled():
        return fantasy_default_state()
    if not FANTASY_STATE_FILE.exists():
        state = fantasy_default_state()
        save_fantasy_state(state)
        return state
    try:
        state = json.loads(FANTASY_STATE_FILE.read_text(encoding="utf-8"))
    except Exception:
        state = fantasy_default_state()
    base = fantasy_default_state()
    for key, value in base.items():
        state.setdefault(key, value)
    for sid, value in base["region_states"].items():
        state["region_states"].setdefault(sid, value)
    return state


def save_fantasy_state(state):
    if not fantasy_enabled():
        return
    FANTASY_STATE_FILE.write_text(json.dumps(state, ensure_ascii=False, indent=2), encoding="utf-8")


def fantasy_log(state, text):
    if not text:
        return
    chronicle = state.setdefault("chronicle", [])
    chronicle.append(text)
    del chronicle[:-40]


def fantasy_region_state(player, state):
    sid = player.current_sector_id or DEFAULT_SECTOR_ID
    return state.setdefault("region_states", {}).setdefault(
        sid, {"stability": 80, "threat": 1, "omen": ""}
    )
...
FANTASY_OMENS = {
    "thorn_moon": {
        "name": "Dornenmond",
        "text": "[OMEN] Der Dornenmond steigt auf. Angriffe sind gefährlicher, Heilung schwächer.",
        "atk_mul": 1.12,
        "enemy_mul": 1.08,
        "heal_mul": 0.75,
        "resource_bonus": 0,
    ...
    "crystal_rain": {
        "name": "Kristallregen",
        "text": "[OMEN] Kristallregen fällt. Ressourcenfunde werden reicher.",
        "atk_mul": 1.0,
        "enemy_mul": 1.0,
        "heal_mul": 1.0,
        "resource_bonus": 1,
    ...
    "blood_star": {
        "name": "Blutstern",
        "text": "[OMEN] Der Blutstern brennt. Kritische Treffer und Feindzorn steigen.",
        "atk_mul": 1.18,
        "enemy_mul": 1.15,
        "heal_mul": 0.9,
        "resource_bonus": 0,
    ...
    "aurora_grace": {
        "name": "Aurora-Segen",
        "text": "[OMEN] Aurora-Segen legt sich über Glutheim. Heilung und Stabilität steigen.",
        "atk_mul": 1.0,
        "enemy_mul": 0.9,
        "heal_mul": 1.25,
        "resource_bonus": 1,
...
FANTASY_RELICS = {
    "seven_sparks_crown": {
        "name": "Krone der Sieben Funken",
        "desc": "+40 Angriff im Kampf, aber der Weltenriss wächst nach Siegen.",
        "atk_add": 40,
        "rupture_on_win": 2,
    },
    "first_moon_mirror": {
        "name": "Spiegel des Ersten Mondes",
        "desc": "Chance, Feindschaden zurückzuwerfen; Heilung wirkt schwächer.",
        "reflect": 0.25,
        "heal_mul": 0.7,
    },
    "dusk_heart": {
        "name": "Herz der Dämmerung",
        "desc": "Rettet dich einmal vor dem Tod, zerbricht danach.",
        "death_save": True,
    },
}

FANTASY_TITLES = {
    "000F01": {"title": "Erwachter Hüter", "max_hp": 5},
    "000F02": {"title": "Runenmeister", "resource_bonus": 1},
    "000F03": {"title": "Lumina-Archon", "atk_add": 5},
}

FANTASY_RIDDLES = [
    {
        "id": "fire_moon_shadow",
        "text": "Drei Runen leuchten: Feuer, Mond, Schatten. 'Was brennt, darf nicht zuerst fallen.'",
        "choices": ["Feuer → Mond → Schatten", "Mond → Feuer → Schatten", "Schatten → Feuer → Mond"],
        "answer": "2",
        "reward": "000N55",
    },
    {
        "id": "silent_gate",
        "text": "Ein Tor fragt: 'Ich öffne mich nur, wenn niemand mich befiehlt. Was bin ich?'",
        "choices": ["Ein Schlüssel", "Ein Echo", "Ein Geheimnis"],
        "answer": "3",
        "reward": "000N56",
    },
]

ENEMY_BEHAVIORS = ["berserker", "healer", "shield", "curse", "thief", "splitter"]
BEHAVIOR_NAMES = {
    "berserker": "Berserker",
    "healer": "Regenerator",
    "shield": "Schildträger",
    "curse": "Fluchwirker",
    "thief": "Golddieb",
    "splitter": "Splitterwesen",
}


def fantasy_start_session(engine, player):
    if not fantasy_enabled():
        return
    state = load_fantasy_state()
    if random.random() < 0.35 and not state.get("current_omen"):
        omen_id = random.choice(list(FANTASY_OMENS))
        state["current_omen"] = omen_id
        state["omen_turns"] = random.randint(2, 4)
        print(FANTASY_OMENS[omen_id]["text"])
        fantasy_log(state, f"Ein Omen erschien: {FANTASY_OMENS[omen_id]['name']}.")
    refresh_rotating_merchant(state)
    save_fantasy_state(state)


def refresh_rotating_merchant(state):
    day = int(time.time() // 86400)
    if state.get("merchant_day") == day and state.get("merchant_stock"):
        return
    pool = ["000I100", "000I101", "000I102", "000I200", "000I201", "000I300", "000I301", "000I302", "000N55", "000N56", "000N57"]
    random.seed(day)
    state["merchant_stock"] = random.sample(pool, min(5, len(pool)))
    random.seed()
    state["merchant_day"] = day


def fantasy_before_battle(engine, player, is_boss=False):
    if not fantasy_enabled():
        return {}
    state = load_fantasy_state()
    region = fantasy_region_state(player, state)
    state["path_counts"]["scan"] = state["path_counts"].get("scan", 0) + 1
    omen = FANTASY_OMENS.get(state.get("current_omen", ""))
    if omen:
        print(omen["text"])
    if region.get("stability", 80) < 45:
        print("[REGION] Die Region ist brüchig. Gegner treten stärker hervor.")
    if state.get("world_rupture", 0) >= 70:
        print("[WELTENRISS] Der Riss schreit. Feinde erscheinen verzerrt und zäher.")
    ctx = {
        "state": state,
        "omen": omen,
        "region": region,
    ...
    if omen:
        ctx["enemy_mul"] *= omen.get("enemy_mul", 1.0)
        ctx["atk_mul"] *= omen.get("atk_mul", 1.0)
        ctx["heal_mul"] *= omen.get("heal_mul", 1.0)
        ctx["resource_bonus"] += omen.get("resource_bonus", 0)
    if state.get("world_rupture", 0) >= 50:
        ctx["enemy_mul"] *= 1.10
    if region.get("threat", 1) > 1:
        ctx["enemy_mul"] *= 1.0 + (region.get("threat", 1) - 1) * 0.05
    relic = FANTASY_RELICS.get(state.get("equipped_relic", ""))
    if relic:
        ctx["atk_add"] += relic.get("atk_add", 0)
        ctx["heal_mul"] *= relic.get("heal_mul", 1.0)
    synergy = fantasy_synergy(player, state)
    if synergy:
        print(f"[SYNERGIE] {synergy['name']} aktiv: {synergy['text']}")
        ctx["atk_mul"] *= synergy.get("atk_mul", 1.0)
        ctx["resource_bonus"] += synergy.get("resource_bonus", 0)
    title_bonus = fantasy_active_title_bonus(state)
    ctx["atk_add"] += title_bonus.get("atk_add", 0)
    ctx["resource_bonus"] += title_bonus.get("resource_bonus", 0)
    save_fantasy_state(state)
    return ctx


def fantasy_active_title_bonus(state):
    active = state.get("active_title")
    if not active:
        return {}
    for ach, cfg in FANTASY_TITLES.items():
        if cfg["title"] == active:
            return cfg
    return {}


def fantasy_synergy(player, state):
    items = set(player.inventory_tokens) | set(player.equipped_items.values())
    if {"000I100", "000I102", "000N52"}.issubset(items):
        return {"name": "Sternenpakt", "text": "+10% Angriff, +1 Ressourcenfund.", "atk_mul": 1.10, "resource_bonus": 1}
    if {"000I201", "000E11"}.issubset(items) or ("000I302" in items and player.has_trait("000E11")):
        return {"name": "Dunkler Eid", "text": "+25% Angriff, aber die Welt merkt sich deine Macht.", "atk_mul": 1.25}
    return None


def fantasy_prepare_enemy(engine, player, enemy_token, e_name, e_hp, e_atk, is_boss, ctx):
    if not fantasy_enabled():
        return e_name, e_hp, e_atk, {}
    state = ctx.get("state") or load_fantasy_state()
    nemesis = state.get("nemesis") or {}
    behavior = random.choice(ENEMY_BEHAVIORS)
    special = {"behavior": behavior, "round": 0, "shield_used": False, "split_done": False, "analysed": False}
    if nemesis.get("active") and random.random() < 0.25 and not is_boss:
        e_name = nemesis.get("name", e_name) + " [NEMESIS]"
        e_hp = int(e_hp * 1.6)
        e_atk = int(e_atk * 1.35)
        special["nemesis"] = True
        print(f"[NEMESIS] {e_name} kehrt zurück und erkennt dich wieder.")
    if is_boss:
        special["boss_phases"] = ["Prismenschild", "Lichtbruch", "Zerfallende Krone"]
        special["boss_phase"] = 0
        behavior = "boss"
        special["behavior"] = behavior
    else:
        e_name = f"{e_name} ({BEHAVIOR_NAMES.get(behavior, behavior)})"
        print(f"[VERHALTEN] {BEHAVIOR_NAMES.get(behavior, behavior)}: Dieser Gegner kämpft nicht gewöhnlich.")
    e_hp = int(e_hp * ctx.get("enemy_mul", 1.0))
    e_atk = int(e_atk * ctx.get("enemy_mul", 1.0))
    return e_name, e_hp, e_atk, special


def fantasy_status_line(ctx):
    if not fantasy_enabled() or not ctx:
        return
    state = ctx.get("state")
    if not state:
        return
    ...
    active = []
    if omen:
        active.append(omen["name"])
    if state.get("equipped_relic"):
        active.append(FANTASY_RELICS[state["equipped_relic"]]["name"])
    if state.get("class_path"):
        active.append(state["class_path"])
    if active:
        print("[AETHER] " + " | ".join(active))


def fantasy_handle_combat_choice(choice, engine, player, e_name, e_hp, e_atk, special, ctx):
    if not fantasy_enabled():
        return None
    state = ctx.get("state") or load_fantasy_state()
    if choice == "4":
        print("[KONTER-RUNE] Du spannst eine Gegenrune. Der nächste Feindtreffer wird geschwächt.")
        special["counter"] = True
        return {"handled": True, "continue": True}
    if choice == "5":
        relic_id = state.get("equipped_relic")
        if not relic_id:
            print("[RELIKT] Kein Relikt ausgerüstet.")
        else:
            relic = FANTASY_RELICS[relic_id]
            print(f"[RELIKT] {relic['name']} pulsiert: {relic['desc']}")
            if relic_id == "first_moon_mirror":
                special["mirror_ready"] = True
            elif relic_id == "seven_sparks_crown":
                return {"handled": True, "dmg": 35}
            elif relic_id == "dusk_heart":
                print("[RELIKT] Das Herz wartet auf einen tödlichen Moment.")
        return {"handled": True, "continue": True}
    if choice == "6":
        special["analysed"] = True
        print(f"[ANALYSE] {e_name}: Verhalten={special.get('behavior')}, Angriff≈{int(e_atk)}, Restessenz≈{int(e_hp)}.")
        return {"handled": True, "continue": True}
    if choice == "7":
        if random.random() < 0.55:
            print("[RÜCKZUG] Du entkommst durch einen flackernden Seitenpfad.")
            return {"handled": True, "flee": True}
        print("[RÜCKZUG] Der Seitenpfad bricht zusammen!")
        return {"handled": True, "continue": False}
    return None


def fantasy_enemy_round_start(player, special, e_hp, e_atk):
    if not fantasy_enabled() or not special:
        return e_hp, e_atk
    special["round"] = special.get("round", 0) + 1
    behavior = special.get("behavior")
    if behavior == "healer" and special["round"] % 3 == 0:
        e_hp += 12
        print("[GEGNER] Regeneration: Der Feind schließt seine Wunden.")
    if behavior == "berserker" and e_hp < 25:
        e_atk = int(e_atk * 1.25)
        print("[GEGNER] Berserkerwut: Der Feind schlägt wilder zu.")
    if behavior == "curse" and special["round"] == 2:
        player.atk = max(1, player.atk - 1)
        print("[FLUCH] Deine Angriffskraft wird für diese Reise gezeichnet (-1 ATK).")
    return e_hp, e_atk


def fantasy_modify_player_damage(dmg, special):
    if not fantasy_enabled() or not special:
        return dmg
    if special.get("behavior") == "shield" and not special.get("shield_used") and dmg > 0:
        special["shield_used"] = True
        print("[SCHILD] Der erste Treffer wird von Kristallschuppen halbiert.")
        dmg = max(1, dmg // 2)
    if special.get("analysed") and dmg > 0:
        dmg = int(dmg * 1.12)
    return dmg


def fantasy_modify_enemy_hit(enemy_hit, player, special, ctx):
    if not fantasy_enabled() or not special:
        return enemy_hit, 0
    reflected = 0
    if special.get("counter"):
        enemy_hit = max(1, enemy_hit // 2)
        reflected += enemy_hit
        special["counter"] = False
        print("[KONTER-RUNE] Du brichst den Angriff und wirfst einen Teil zurück.")
    state = ctx.get("state") if ctx else None
    relic_id = state.get("equipped_relic") if state else ""
    if relic_id == "first_moon_mirror" and random.random() < FANTASY_RELICS[relic_id]["reflect"]:
        reflected += enemy_hit
        print("[SPIEGEL] Mondlicht wirft den Schaden zurück.")
    return enemy_hit, reflected


def fantasy_after_victory(engine, player, e_name, enemy_token, is_boss, ctx):
    if not fantasy_enabled():
        return
    state = ctx.get("state") or load_fantasy_state()
    region = fantasy_region_state(player, state)
    region["stability"] = max(0, min(100, region.get("stability", 80) + (4 if is_boss else 1)))
    state["world_rupture"] = max(0, state.get("world_rupture", 0) - (5 if is_boss else 1))
    relic = FANTASY_RELICS.get(state.get("equipped_relic", ""))
    if relic and relic.get("rupture_on_win"):
        state["world_rupture"] += relic["rupture_on_win"]
    if ctx.get("resource_bonus", 0):
        for _ in range(int(ctx["resource_bonus"])):
            token = random.choice(["000N50", "000N51", "000N52", "000N55", "000N56", "000N57"])
            player.inventory_tokens.append(token)
        print(f"[BONUS] Omen, Titel oder Synergie schenken dir {int(ctx['resource_bonus'])} zusätzliche Ressource(n).")
    bestiary = state.setdefault("bestiary", {})
    entry = bestiary.setdefault(enemy_token, {"name": e_name, "defeated": 0, "notes": []})
    entry["name"] = e_name
    entry["defeated"] = entry.get("defeated", 0) + 1
    if "Besiegt in " + resolve_player_shard_name(engine, player) not in entry["notes"]:
        entry["notes"].append("Besiegt in " + resolve_player_shard_name(engine, player))
    if state.get("current_omen"):
        state["omen_turns"] = max(0, state.get("omen_turns", 0) - 1)
        if state["omen_turns"] <= 0:
            print("[OMEN] Das Omen verblasst.")
            state["current_omen"] = ""
    fantasy_maybe_unlock_path(state, player)
    fantasy_log(state, f"{player.name} besiegte {e_name} in {resolve_player_shard_name(engine, player)}.")
    if random.random() < (0.18 if is_boss else 0.07):
        available_relics = [rid for rid in FANTASY_RELICS if rid not in state.get("relics", [])]
        if available_relics:
            relic_id = random.choice(available_relics)
            state.setdefault("relics", []).append(relic_id)
            print(f"[RELIKT] Gefunden: {FANTASY_RELICS[relic_id]['name']}")
            fantasy_log(state, f"Ein Relikt wurde geborgen: {FANTASY_RELICS[relic_id]['name']}.")
    save_fantasy_state(state)


def fantasy_after_defeat(player, e_name, ctx):
    if not fantasy_enabled():
        return
    state = ctx.get("state") if ctx else load_fantasy_state()
    state["world_rupture"] = min(100, state.get("world_rupture", 0) + 8)
    state["nemesis"] = {"active": True, "name": e_name, "level": int(player.lvl)}
    fantasy_log(state, f"{e_name} wurde zur Nemesis, nachdem {player.name} fiel.")
    save_fantasy_state(state)


def fantasy_death_save(player, ctx):
    if not fantasy_enabled() or not ctx:
        return False
    state = ctx.get("state") or load_fantasy_state()
    if state.get("equipped_relic") == "dusk_heart":
        player.hp = max(1, int(player.max_hp * 0.35))
        state["equipped_relic"] = ""
        if "dusk_heart" in state.get("relics", []):
            state["relics"].remove("dusk_heart")
        print("[RELIKT] Das Herz der Dämmerung zerbricht und rettet dich vor dem Tod.")
        save_fantasy_state(state)
        return True
    return False


def fantasy_maybe_unlock_path(state, player):
    counts = state.setdefault("path_counts", {})
    if state.get("class_path"):
        return
    candidates = [
        ("Runenberserker", counts.get("heavy", 0), 4),
        ("Lichtwächter", counts.get("heal", 0), 3),
        ("Sternenschmied", counts.get("craft", 0), 2),
        ("Omenbinder", counts.get("event", 0), 2),
        ("Gesandter", counts.get("faction", 0), 5),
    ]
    for name, value, threshold in candidates:
        if value >= threshold:
            state["class_path"] = name
            print(f"[PFAD ERWACHT] Du bist nun ein {name}.")
            fantasy_log(state, f"Ein Klassenpfad erwachte: {name}.")
            return


def fantasy_record_action(kind, amount=1):
    if not fantasy_enabled():
        return
    state = load_fantasy_state()
    counts = state.setdefault("path_counts", {})
    counts[kind] = counts.get(kind, 0) + amount
    fantasy_maybe_unlock_path(state, Player("Operator"))
    save_fantasy_state(state)


def fantasy_available_titles(player):
    titles = []
    for ach, cfg in FANTASY_TITLES.items():
        if ach in player.achievements:
...
def handle_market_action(engine: I18nEngine, player: Player):
    ...
    if fantasy_enabled():
        ...
        refresh_rotating_merchant(state)
        save_fantasy_state(state)
        print("[WANDERHÄNDLER] Heutiges Sonderangebot:")
        for token in state.get("merchant_stock", []):
            print(" - " + get_token_label(engine, token))
    while True:
        ...
        if choice == "1":
            ...
            break
        elif choice == "4" and fantasy_enabled():
            state = load_fantasy_state()
            stock = list(state.get("merchant_stock", []))
            if not stock:
                print("[WANDERHÄNDLER] Heute ist der Koffer leer.")
                continue
            for idx, token in enumerate(stock, start=1):
                base = price_map["items"].get(token, price_map["resources"].get(token, RESOURCE_BASE_VALUES.get(token, 180)))
                price = max(1, int(round(base * 1.15 + extra)))
                print(f"({idx}) {get_token_label(engine, token)} — {price} Gold")
            sel = input("> ").strip()
            if sel.isdigit() and 0 <= int(sel) - 1 < len(stock):
                token = stock[int(sel) - 1]
                base = price_map["items"].get(token, price_map["resources"].get(token, RESOURCE_BASE_VALUES.get(token, 180)))
                price = max(1, int(round(base * 1.15 + extra)))
                if player.credits >= price:
                    player.credits -= price
                    player.inventory_tokens.append(token)
                    print(f"[WANDERHÄNDLER] Gekauft: {get_token_label(engine, token)}")
                    fantasy_log(state, f"Beim Wanderhändler wurde {get_token_label(engine, token)} gekauft.")
                    save_fantasy_state(state)
                else:
                    print(engine.translate("000C26", [price]))
        else:
...
def handle_event_action(engine: I18nEngine, player: Player):
    fantasy_record_action("event")
    world_event_menu(engine, player)


@register_menu_action("REST")
def handle_rest_action(engine: I18nEngine, player: Player):
    if not fantasy_enabled():
        print("Rast ist nur im Fantasy-Katalog verfügbar.")
        return
    state = load_fantasy_state()
    print("\n--- LAGERFEUER ---")
    print("(1) Kurz rasten: +30 HP, Weltenriss +1")
    print("(2) Tief schlafen: volle Heilung, Bedrohung steigt")
    print("(3) Nachtwache: Chance auf Omen oder Schatz")
    choice = input("> ").strip()
    region = fantasy_region_state(player, state)
    if choice == "1":
        player.hp = min(player.max_hp, player.hp + 30)
        state["world_rupture"] = min(100, state.get("world_rupture", 0) + 1)
        print("[RAST] Du wärmst dich am Runenfeuer.")
    elif choice == "2":
        player.hp = player.max_hp
        region["threat"] = min(5, region.get("threat", 1) + 1)
        print("[RAST] Du schläfst tief. Etwas in der Region erwacht.")
    elif choice == "3":
        if random.random() < 0.5:
            token = random.choice(["000N55", "000N56", "000N57"])
            player.inventory_tokens.append(token)
            print("[NACHTWACHE] Du findest eine seltene Ressource.")
        else:
            state["current_omen"] = random.choice(list(FANTASY_OMENS))
            state["omen_turns"] = 3
            print(FANTASY_OMENS[state["current_omen"]]["text"])
    fantasy_log(state, "Eine Rast am Lagerfeuer veränderte die Reise.")
    save_fantasy_state(state)


@register_menu_action("EXP")
def handle_expedition_action(engine: I18nEngine, player: Player):
    if not fantasy_enabled():
        print("Expeditionen sind nur im Fantasy-Katalog verfügbar.")
        return
    state = load_fantasy_state()
    print("\n--- EXPEDITION ---")
    print("Eine Expedition besteht aus drei Kammern: Kampf, Ereignis/Rätsel und Boss.")
    print("(1) Mondkatakomben betreten")
    print("(2) Kristallgruft betreten")
    print("(3) Zurück")
    choice = input("> ").strip()
    if choice not in {"1", "2"}:
        return
    state["expedition_active"] = True
    fantasy_log(state, "Eine Expedition wurde begonnen.")
    save_fantasy_state(state)
    for depth in range(1, 4):
        print(f"\n[EXPEDITION] Kammer {depth}/3")
        if depth == 1:
            run_battle(player, engine)
        elif depth == 2:
            if random.random() < 0.5:
                handle_riddle_action(engine, player)
            else:
                token = random.choice(["000N55", "000N56", "000N57"])
                player.inventory_tokens.append(token)
                print("[SCHATZ] Eine versiegelte Truhe gibt eine seltene Ressource frei.")
        else:
            run_battle(player, engine, True)
        if player.hp <= 0:
            break
    ...
    fantasy_log(state, "Die Expedition endete.")
    save_fantasy_state(state)


@register_menu_action("BEST")
def handle_bestiary_action(engine: I18nEngine, player: Player):
    if not fantasy_enabled():
        print("Bestiarium ist nur im Fantasy-Katalog verfügbar.")
        return
    state = load_fantasy_state()
    print("\n--- BESTIARIUM ---")
    bestiary = state.get("bestiary", {})
    if not bestiary:
        print("Noch keine Wesen verzeichnet.")
        return
    for idx, (token, entry) in enumerate(sorted(bestiary.items()), start=1):
        notes = "; ".join(entry.get("notes", [])[:2])
        print(f"{idx}. {entry.get('name', token)} | Besiegt: {entry.get('defeated', 0)} | {notes}")


@register_menu_action("TITLE")
def handle_title_action(engine: I18nEngine, player: Player):
    if not fantasy_enabled():
        print("Titel sind nur im Fantasy-Katalog verfügbar.")
        return
    state = load_fantasy_state()
    titles = fantasy_available_titles(player)
    print("\n--- TITEL ---")
    if not titles:
        print("Noch keine Titel freigeschaltet.")
        return
    for idx, title in enumerate(titles, start=1):
        marker = " *" if state.get("active_title") == title else ""
        print(f"({idx}) {title}{marker}")
    choice = input("> ").strip()
    if choice.isdigit() and 0 <= int(choice) - 1 < len(titles):
        state["active_title"] = titles[int(choice) - 1]
        print(f"[TITEL] Aktiv: {state['active_title']}")
        save_fantasy_state(state)


@register_menu_action("LCHRON")
def handle_living_chronicle_action(engine: I18nEngine, player: Player):
    if not fantasy_enabled():
        print("Lebende Chronik ist nur im Fantasy-Katalog verfügbar.")
        return
    state = load_fantasy_state()
    print("\n--- LEBENDE CHRONIK ---")
    print(f"Weltenriss: {state.get('world_rupture', 0)}%")
    print(f"Pfad: {state.get('class_path') or 'noch unbestimmt'}")
    print(f"Aktives Omen: {FANTASY_OMENS.get(state.get('current_omen',''), {}).get('name', 'keines')}")
    print("")
    for line in state.get("chronicle", [])[-12:]:
        print("- " + line)


@register_menu_action("RELIC")
def handle_relic_action(engine: I18nEngine, player: Player):
    if not fantasy_enabled():
        print("Relikte sind nur im Fantasy-Katalog verfügbar.")
        return
    state = load_fantasy_state()
    print("\n--- RELIKTE ---")
    relics = state.get("relics", [])
    if not relics:
        print("Noch keine Relikte gefunden. Bosse, Expeditionen und Chroniken können Relikte offenbaren.")
        return
    for idx, rid in enumerate(relics, start=1):
        r = FANTASY_RELICS[rid]
        marker = " *" if state.get("equipped_relic") == rid else ""
        print(f"({idx}) {r['name']}{marker} — {r['desc']}")
    choice = input("> ").strip()
    if choice.isdigit() and 0 <= int(choice) - 1 < len(relics):
        state["equipped_relic"] = relics[int(choice) - 1]
        print(f"[RELIKT] Ausgerüstet: {FANTASY_RELICS[state['equipped_relic']]['name']}")
        save_fantasy_state(state)


@register_menu_action("OMEN")
def handle_omen_action(engine: I18nEngine, player: Player):
    if not fantasy_enabled():
        print("Omen sind nur im Fantasy-Katalog verfügbar.")
        return
    state = load_fantasy_state()
    print("\n--- OMENBINDER ---")
    print(f"Weltenriss: {state.get('world_rupture', 0)}%")
    current = FANTASY_OMENS.get(state.get("current_omen", ""))
    print("Aktiv:", current["name"] if current else "keines")
    print("(1) Omen lesen")
    print("(2) Neues Omen erzwingen (+5 Weltenriss)")
    print("(3) Omen bannen (-150 Gold)")
    choice = input("> ").strip()
    if choice == "1" and current:
        ...
        state["current_omen"] = random.choice(list(FANTASY_OMENS))
        state["omen_turns"] = 3
        state["world_rupture"] = min(100, state.get("world_rupture", 0) + 5)
        state["path_counts"]["event"] = state["path_counts"].get("event", 0) + 1
        print(FANTASY_OMENS[state["current_omen"]]["text"])
    elif choice == "3" and player.credits >= 150:
        player.credits -= 150
        state["current_omen"] = ""
        state["omen_turns"] = 0
        print("[OMEN] Das Zeichen wird in Salz gebannt.")
    fantasy_maybe_unlock_path(state, player)
    save_fantasy_state(state)


@register_menu_action("RIDDLE")
def handle_riddle_action(engine: I18nEngine, player: Player):
    if not fantasy_enabled():
        print("Rätsel sind nur im Fantasy-Katalog verfügbar.")
        return
    state = load_fantasy_state()
    riddle = random.choice(FANTASY_RIDDLES)
    print("\n--- RUNENRÄTSEL ---")
    print(riddle["text"])
    for idx, label in enumerate(riddle["choices"], start=1):
        print(f"({idx}) {label}")
    answer = input("> ").strip()
    if answer == riddle["answer"]:
        player.inventory_tokens.append(riddle["reward"])
        state["world_rupture"] = max(0, state.get("world_rupture", 0) - 2)
        print("[RÄTSEL] Die Runen öffnen sich. Du erhältst eine seltene Ressource.")
        fantasy_log(state, "Ein Runenrätsel wurde gelöst.")
    else:
        state["world_rupture"] = min(100, state.get("world_rupture", 0) + 3)
        print("[RÄTSEL] Die Runen schneiden in die Luft. Der Weltenriss wächst.")
        fantasy_log(state, "Ein Runenrätsel scheiterte.")
    save_fantasy_state(state)
...
def run_battle(player, engine, is_boss=False):
    # GPU-CUBE PHYSIK LOGIK + Fantasy Expansion Hooks
    stability = random.randint(50, 100)
    physics_bonus = 0
    fantasy_ctx = fantasy_before_battle(engine, player, is_boss)
    if is_boss:
    ...
    e_name, e_hp, e_atk, fantasy_enemy = fantasy_prepare_enemy(
        engine, player, enemy_token, e_name, e_hp, e_atk, is_boss, fantasy_ctx
    )
    original_hp = e_hp
    print(
        engine.translate(
    ...
    while e_hp > 0 and player.hp > 0:
        e_hp, e_atk = fantasy_enemy_round_start(player, fantasy_enemy, e_hp, e_atk)
        if is_boss and not boss_phase_triggered and e_hp <= original_hp / 2:
            ...
            if fantasy_enabled():
                print("[BOSS-PHASE] Der Boss zerbricht seine erste Maske. Lichtbruch beginnt.")
        if is_boss and fantasy_enemy and e_hp <= original_hp * 0.25 and fantasy_enemy.get("boss_phase", 0) < 2:
            fantasy_enemy["boss_phase"] = 2
            e_atk = int(e_atk * 1.25)
            print("[BOSS-PHASE] Zerfallende Krone: Der Boss wird verwundbar, aber grausamer.")
        print(
            translate_with_fallback(
                engine,
        ...
        fantasy_status_line(fantasy_ctx)
        prompt = engine.translate("000201")
        if fantasy_enabled():
            prompt += " (4=Konter-Rune) (5=Relikt) (6=Analyse) (7=Rückzug)"
        if skill_ready:
            prompt += " (S=Skill)"
        ...
        fantasy_action = fantasy_handle_combat_choice(choice, engine, player, e_name, e_hp, e_atk, fantasy_enemy, fantasy_ctx)
        if fantasy_action:
            if fantasy_action.get("flee"):
                return True
            if fantasy_action.get("dmg"):
                e_hp -= fantasy_action["dmg"]
                print(engine.translate("000301", [player.name, e_name, int(fantasy_action["dmg"])]))
                if e_hp <= 0:
                    break
            if fantasy_action.get("continue"):
                continue

        dmg = 0
        atk_add = item_effects.get("atk_add", 0) + fantasy_ctx.get("atk_add", 0)
        atk_mul = item_effects.get("atk_mul", 1.0) * fantasy_ctx.get("atk_mul", 1.0)
        ...
        if choice == "1":
            ...
            fantasy_record_action("heavy")
            print(translate_with_fallback(engine, UI_TOKENS["full_hit"]))
        elif choice == "3":
            heal_amount = int(25 * fantasy_ctx.get("heal_mul", 1.0))
            player.hp = min(player.max_hp, player.hp + heal_amount)
            fantasy_record_action("heal")
            print(translate_with_fallback(engine, UI_TOKENS["repair_action"]))

        if dmg > 0:
            dmg = fantasy_modify_player_damage(dmg, fantasy_enemy)
        ...
        if e_hp > 0:
            ...
            if fantasy_enemy.get("behavior") == "thief" and random.random() < 0.18 and player.credits > 0:
                stolen = min(player.credits, random.randint(5, 25))
                player.credits -= stolen
                print(f"[DIEB] {e_name} stiehlt {stolen} Goldmünzen.")
            enemy_hit, reflected = fantasy_modify_enemy_hit(enemy_hit, player, fantasy_enemy, fantasy_ctx)
            player.hp -= enemy_hit
            if reflected:
                e_hp -= reflected
                print(engine.translate("000301", [player.name, e_name, int(reflected)]))
            print(engine.translate("000301", [e_name, player.name, int(enemy_hit)]))

    if player.hp <= 0:
        if fantasy_death_save(player, fantasy_ctx):
            return True
        fantasy_after_defeat(player, e_name, fantasy_ctx)
        return False
    ...
    if e_hp <= 0:
        ...
        if is_boss:
            ...
            fantasy_after_victory(engine, player, e_name, enemy_token, is_boss, fantasy_ctx)
            return "WIN"
...
def main():
    ...
    fantasy_start_session(engine, player)
